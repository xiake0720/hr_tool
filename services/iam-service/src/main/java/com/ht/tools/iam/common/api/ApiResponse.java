package com.ht.tools.iam.common.api;

import com.ht.tools.iam.common.web.RequestIdContext;
 ...
 * 与其它组件关系：依赖 {@link RequestIdContext} 提供 requestId，与 {@code ErrorCode} 约定错误码。
...
public class ApiResponse<T> {
     ...
     * 入参：data - 业务数据。
     * 返回：成功响应体。
     * 副作用：会读取或生成 {@link RequestIdContext} 中的 requestId。
     * 是否可空：data 可空。
     * 异常：无。
     * 线程安全：线程安全（每次调用创建新对象）。
    ...
    public static <T> ApiResponse<T> ok(T data) {
        return new ApiResponse<>("SYS_0000", "OK", data, RequestIdContext.getOrCreate());
    }
    ...
    public static ApiResponse<Void> ok() {
        return new ApiResponse<>("SYS_0000", "OK", null, RequestIdContext.getOrCreate());
    }
    ...
    public static <T> ApiResponse<T> fail(String code, String message) {
        return new ApiResponse<>(code, message, null, RequestIdContext.getOrCreate());
    }
     ...
     * 异常：无。
     * 线程安全：非线程安全（修改状态）。
     * 使用建议：优先由 {@link RequestIdContext} 提供，避免手动拼装。
     *
     * @param requestId 请求追踪 ID
     */
